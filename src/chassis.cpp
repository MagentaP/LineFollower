...
void Chassis::setVelocity(float v, float w)
{
    ...
    if (dr < 0 && dr > -duty_bias) dr = -duty_bias;
    left_.setDuty(dl); right_.setDuty(dr);
}
...
{
    pid_yaw_.kp_ = yaw_kp;  pid_yaw_.ki_ = yaw_ki;
    pid_yaw_.kd_ = yaw_kd;  pid_yaw_.i_limit_ = yaw_i_limit;
    pid_yaw_.o_limit_ = max_angular_velocity;
}

void Chassis::applyOutput_() { setVelocity(vel_, omega_); }
...
void Chassis::dispatchMode_()
{
    if (web_mode_ != prev_mode_)
    {
        ...
        lock_count_ = 0; prev_mode_ = web_mode_;
    }
    if      (web_mode_ == "DEBUG")  handleDebug_();
    else if (web_mode_ == "STOP")   handleStop_();
    else if (web_mode_ == "MANUAL") handleManual_();
    else                            handleAuto_();
    state_str_ = (web_mode_=="AUTO"||web_mode_=="DEBUG") ? autoStateName_(auto_state_) : web_mode_;
}
...
void Chassis::handleDebug_()
{
    ...
    if (fabs(debug_angle_) < 1.0f)
    {
        ...
        if (millis() - st > 1000) { yaw_ref_ += 90; wrapAngle_(yaw_ref_); st = millis(); }
    }
    else yaw_ref_ = debug_angle_;
...
void Chassis::handleManual_()
{
    ...
    else if (web_cmd_=="RIGHT") { man_ref_-=1.57f*0.01f*R2D; wrapAngle_(man_ref_); }
    yaw_ref_ = man_ref_;
    if (web_cmd_=="FWD") vel_=target_speed; else if (web_cmd_=="BACK") vel_=-target_speed; else vel_=0;
    omega_ = runYawPid_(yaw_ref_);
    auto_state_ = AUTO_LOCKED;
}

void Chassis::handleAuto_()
{ switch(auto_state_){ case AUTO_NORMAL:autoNormal_();break; case AUTO_GAP:autoGap_();break; case AUTO_LOST:autoLost_();break; case AUTO_LOCKED:autoLocked_();break; } }

void Chassis::autoNormal_()
{
    if (conf_ >= line_confidence_threshold)
    { last_pos_=pos_; yaw_ref_=-pos_*60.0f; vel_=target_speed; omega_=runYawPid_(yaw_ref_); }
    else { enterAutoState_(AUTO_GAP); last_omega_=(omega_=last_omega_*0.5f); vel_=target_speed*0.3f; }
}

void Chassis::autoGap_()
{
    if (conf_ >= line_confidence_threshold)
    { enterAutoState_(AUTO_NORMAL); yaw_ref_=-pos_*60.0f; vel_=target_speed; omega_=runYawPid_(yaw_ref_); return; }
    bool maybe=false; for(int i=1;i<=2;i++) if(raw_buf_[i]>1200) maybe=true;
    if (millis()-gap_time_ < (maybe?gap_grace_ms*3U:(unsigned long)gap_grace_ms))
    { omega_=last_omega_*0.5f; vel_=target_speed*0.3f; }
    else { enterAutoState_(AUTO_LOST); recovery_sign_=last_pos_<0?1:-1; omega_=0; }
}

void Chassis::autoLost_()
{
    if (conf_ >= line_confidence_threshold)
    { enterAutoState_(AUTO_NORMAL); yaw_ref_=-pos_*60.0f; vel_=target_speed; omega_=runYawPid_(yaw_ref_); }
    else if (millis()-lost_time_ >= (unsigned long)lost_recovery_timeout_ms) { enterAutoState_(AUTO_LOCKED); }
    else { float p=(float)(millis()-lost_time_)/lost_recovery_timeout_ms; omega_=recovery_sign_*(0.3f+0.7f*p)*max_angular_velocity; vel_=target_speed*0.3f; }
}
